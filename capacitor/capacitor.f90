...
! Globale Variablen: 
  ...
  double precision :: delta, deltamx, U, resulting
  double precision, allocatable,dimension(:) :: Phi,Phi_old
...
if(myid==0) then
  ...
  allocate(Phi_old(0:dim))
  call InitPhi(Phi_old) ! Initialisiere Phi und setze Randwerte 
  Phi = Phi_old
!  Phi_old(0) = 1.d0
     ...
     do k=0, n_r-1
        ...
        do i=1, n_z-1
           ...
           if (k==0) then ! Zentrale Achse ist speziell: 
              U = ( Phi_old(idx(i+1,k)) + Phi_old(idx(i-1,k)) + 4 * Phi_old(idx(i,k+1)) ) / 6.0
           else  ! Punkte im Innern sind standard: 
              U = (   Phi_old(idx(i,k+1)) + Phi_old(idx(i,k-1)) + Phi_old(idx(i+1,k)) + Phi_old(idx(i-1,k)) ) / 4.0 &
                   & + ( Phi_old(idx(i,k+1)) - Phi_old(idx(i,k-1)) ) / ( 8.0 * k )
           end if
!           Phi_old = Phi(idx(i,k))
           Phi(idx(i,k)) = Phi_old(idx(i,k)) + w * ( U - Phi_old(idx(i,k)))
           ! Berechne maximale Abweichung: 
           delta = abs(Phi(idx(i,k))-Phi_old(idx(i,k)))
